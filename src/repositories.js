...
export class NotificationMessageRepository {
    ...
    async getNotificationById(notificationId) {
        const queryString = `SELECT * FROM NotificationMessage WHERE notificationId='${notificationId}'`;
...
export class JobHubNotificationsRepository {
    ...
    async createNewNotification(userId) {
    ...
    async deleteNotification(connectionId) {
        const queryString = `DELETE FROM JobHubConnections WHERE connectionId='${connectionId}'`;
        const result = await this.query(queryString);
    }

    async getNotificationByUserId(userId) {
        const queryString = `SELECT * FROM JobHubConnections WHERE userId='${userId}'`;
        return (await this.query(queryString))[0];
    }

    async getNotificationByConnectionId(connectionId) {
        const queryString = `SELECT * FROM JobHubConnections WHERE connectionId='${connectionId}'`;
        return (await this.query(queryString))[0];
    }
}